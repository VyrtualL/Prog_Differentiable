...
# Autograd Engine, Optimizers & Schedulers From Scratch

*Authors : Matis Braun and Virgile Hermant*
...
`backward()` builds a topological ordering of the computation graph (DFS) and replays it in reverse, applying the chain rule at each node to accumulate gradients (the same principle PyTorch's autograd is built on), here implemented for scalar/elementwise operations only.

## Optimizers

We coded five optimizers on top of the `Tensor`/`Optimizer` classes, each with the standard update rule :
...
To actually see the difference, each optimizer is run on a convex function `(x-2)²` and a non-convex one `3x² - 2x`, starting from the same point. We animate the trajectories (gif) and also plot the loss curves side by side 

With the results, it's pretty obvious which ones overshoot, which ones are slow, ...
...
Re-evaluated on the same convex/non-convex functions and on the neural network training loop, to compare convergence with and without scheduling.
